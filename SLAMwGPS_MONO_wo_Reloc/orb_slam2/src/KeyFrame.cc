...
// // *** Don't have to add distance weight. Distance weight is already update in teh mConnectedKeyFrameWeights.
// void KeyFrame::UpdateBestCovisibles()
// {
//     unique_lock<mutex> lock(mMutexConnections);
//     vector<pair<int,KeyFrame*>> vPairs;
...
int KeyFrame::MakeFinalWeight(int weight, const std::pair<double, double> &mGPS)
{
    ...
    double a = distance / 0.5;
    int dWeight = 100 * inv_sqrt_2pi / 0.5 * std::exp(-0.5 * a * a);
...
void KeyFrame::serialize(Archive &ar, const unsigned int version)
{
    ...
    ar & const_cast<std::pair<double,double> &>(mkGPS);
