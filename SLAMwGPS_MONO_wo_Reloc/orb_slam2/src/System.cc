               ...
               map_file(map_file), load_map(load_map)
{
    ...
    // load serialized map
     if (load_map && LoadMap(map_file)) {
        ...
        std::vector<MapPoint*> coutMappoints = GetAllMapPoints();
        std::vector<KeyFrame*> mapKeyFrames = mpMap->GetAllKeyFrames();

        ofstream myfile("/home/jiwon/catkin_ws/mapPoints_"+map_file+".txt"); 
        for (vector<MapPoint*>::iterator vitMP=coutMappoints.begin(), vendMP=coutMappoints.end(); vitMP!=vendMP; vitMP++)
        {
            MapPoint* pMP = *vitMP;
            cv::Mat x_y = pMP -> GetWorldPos();
            myfile << x_y.at<float>(0) << " " << x_y.at<float>(1) << " " << x_y.at<float>(2) << "\n";            
        }
        myfile.close();
        
        ofstream covfile("/home/jiwon/catkin_ws/covisibility.txt");
        
        KeyFrame* baseKeyFrame =mapKeyFrames[0];
        cv::Mat base_camera_center = pMP -> GetCameraCenter();
        covfile << base_camera_center.at<float>(0,0) << " " << base_camera_center.at<float>(0,1) << " " << base_camera_center.at<float>(0,2) << "\n";
        
        std::vector<KeyFrame*> OrderedKeyFrames = baseKeyFrame -> GetVectorCovisibleKeyFrames();
        for (size_t i=0; i<OrderedKeyFrames.size();i++)
        {
            KeyFrame* pMP = OrderedKeyFrames[i];
            cv::Mat camera_center = pMP -> GetCameraCenter();
            covfile << camera_center.at<float>(0,0) << " " << camera_center.at<float>(0,1) << " " << camera_center.at<float>(0,2) << "\n";
        }
        covfile.close();    

        
        ofstream keyfile("/home/jiwon/catkin_ws/keyframedata_"+map_file+".txt");
        for (size_t i=0; i<mapKeyFrames.size();i++)
        {
            KeyFrame* pMP = mapKeyFrames[i];
            cv::Mat camera_center = pMP -> GetCameraCenter();
            cv::Mat R = pMP -> GetRotation();
            ...
            bool isRotationMatrix = norm(I, shouldBeIdentity) < 1e-6;
            ...
            // keyfile << key_pose.at<float>(0,0) << " " << key_pose.at<float>(0,1) << " " << key_pose.at<float>(0,2) << " " << key_pose.at<float>(1,0) << " " << key_pose.at<float>(1,1) << " " << key_pose.at<float>(1,2) << " " << key_pose.at<float>(2,0) << " " << key_pose.at<float>(2,1) << " " << key_pose.at<float>(2,2) << "\n";
            keyfile << camera_center.at<float>(0) << " " << camera_center.at<float>(1) << " " << camera_center.at<float>(2)<<" " << x <<" "<< y <<" "<< z <<" "<<(float)(pMP -> GetGPSData().first) << " " << (float)(pMP -> GetGPSData().second)<<"\n";
            // keyfile << pMP -> GetGPSData().first << " " << pMP -> GetGPSData().second << " " << "\n";            
        }
        keyfile.close();
        
        ofstream trackframe("/home/jiwon/catkin_ws/tracknum_"+map_file+".txt");
        trackframe<<"NonTracking Frame Num: "<< mpMap->GetNframe()<<" Actual Tracking Frame Num: "<< mpMap->GetTframe()<<" Global BA Num: "<<mpMap->GetGnum()<<" All Frame Num: "<<mpMap->GetFframe()<<" Loop closing Num: "<<mpMap->GetCnum()<<" Relocalization Num: "<<mpMap->GetRnum()<<" NonRelocalization Num: "<<mpMap->GetNRnum()<<"\n";
        trackframe.close();
        cout << "Finish writing text file" << endl;
        //
...
void System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const double &mLatitude, const double &mLongitude)
{
    if(mSensor!=STEREO)
    {
    ...
    cv::Mat Tcw = mpTracker->GrabImageStereo(imLeft,imRight,timestamp,mLatitude,mLongitude);
...
bool System::SaveMap(const string &filename) {
    ...
    std::string file = "/home/jiwon/mapfile/"+filename+".bin";
...
bool System::LoadMap(const string &filename) {
    ...
    unique_lock<mutex>MapPointGlobal(MapPoint::mGlobalMutex);
    std::string file = "/home/jiwon/catkin_ws/map_file/"+filename+".bin";
