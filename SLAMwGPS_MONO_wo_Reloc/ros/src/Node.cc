...
void Node::Update () {
  ...
  if(updateNum ==5000)
  {
    SaveMapSrvFile("/home/jiwon/mapfile/map_save/"+map_file_name_param_+".bin");
    updateNum = 0;
  }
...
bool Node::SaveMapSrvFile (std::string name) {

  bool success = orb_slam_->SaveMap(name);

  if (success) {
    std::cout<<"Map was saved as " << name << std::endl;
  } else {
    std::cout<<"Map could not be saved."<< std::endl;
  }
...
void Node::LoadOrbParameters (ORB_SLAM2::ORBParameters& parameters) {
  ...
  node_handle_.param(name_of_node_ + "/camera_fps", parameters.maxFrames, 15);
