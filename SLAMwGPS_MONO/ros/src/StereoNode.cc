...
StereoNode::StereoNode (const ORB_SLAM2::System::eSensor sensor, ros::NodeHandle &node_handle, image_transport::ImageTransport &image_transport) : Node (sensor, node_handle, image_transport) {
    left_sub_ = new message_filters::Subscriber<sensor_msgs::Image> (node_handle, "image_left/image_color_rect", 100);
    right_sub_ = new message_filters::Subscriber<sensor_msgs::Image> (node_handle, "image_right/image_color_rect", 100);
    gps_subscriber_ = new message_filters::Subscriber<sensor_msgs::NavSatFix> (node_handle, "/fix", 100);
    camera_info_topic_ = "/camera/infra1/camera_info";
    gps_subscriber_->registerCallback(boost::bind(&StereoNode::GpsCallBack, this, _1));
...
void StereoNode::ImageCallback (const sensor_msgs::ImageConstPtr& msgLeft, const sensor_msgs::ImageConstPtr& msgRight) {
  ...
  orb_slam_->TrackStereo(cv_ptrLeft->image, cv_ptrRight->image, cv_ptrLeft->header.stamp.toSec(),latitude,longitude);

  Update ();
}

void StereoNode::GpsCallBack(const sensor_msgs::NavSatFix::ConstPtr& msgG)
{
